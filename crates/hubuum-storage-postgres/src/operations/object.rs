...
impl<'value> From<&'value StorageObjectUpdate> for UpdateObjectRow<'value> {
    fn from(update: &'value StorageObjectUpdate) -> Self {
        Self {
            name: update.name(),
            collection_id: update.collection_id().map(hubuum_domain::CollectionId::id),
            hubuum_class_id: update.class_id().map(ClassId::id),
            data: update.data(),
...
pub(crate) async fn resolve_object_on(
    ...
    crate::validate_persisted(
        "resolved object",
        StorageResolvedObject::try_new(selector, class.into_storage()?, object.into_storage()?),
    )
}
...
pub async fn create_object(
    ...
    runtime
        .with_transaction(async move |connection| {
            create_object_on(&operation_runtime, connection, &target, command, &context).await
        })
        .await
}
...
pub async fn update_object(
    ...
    runtime
        .with_transaction(async move |connection| {
            update_object_on(&operation_runtime, connection, &target, changes, &context).await
        })
        .await
}
...
pub async fn delete_object(
    ...
    runtime
        .with_transaction(async move |connection| {
            delete_object_on(connection, &target, &context).await
        })
        .await
}
...
async fn insert_object(
    ...
    diesel::insert_into(crate::schema::hubuumobject::table)
        .values((
            ...
            hubuum_class_id.eq(command.class_id().id()),
...
async fn lock_resolved_object(
    ...
    let locked_class = match target.selector() {
        StorageObjectSelector::Ids { class_id, .. } => crate::schema::hubuumclass::table
            ...
            .filter(crate::schema::hubuumclass::id.eq(resolved_class.id().id()))
            .filter(crate::schema::hubuumclass::name.eq(resolved_class.name()))
            .filter(
                crate::schema::hubuumclass::collection_id.eq(resolved_class.collection_id().id()),
            )
            .for_update()
...
async fn persist_object_update(
    ...
    (
        StorageMutationOutcome<ObjectRow>,
        Option<ComputedEvaluationSummary>,
    ),
...
> {
    ...
    if !update.changes(&before) {
        ...
        .await?;
        return Ok((StorageMutationOutcome::unchanged(before), evaluation));
    }
    let updated = diesel::update(
        crate::schema::hubuumobject::table.filter(crate::schema::hubuumobject::id.eq(before.id)),
    )
...
fn validate_object_update(
    ...
    validate_object_state(
        changes
            .class_id()
            .map(ClassId::id)
            .unwrap_or(current.hubuum_class_id),
        changes
            .collection_id()
            .map(hubuum_domain::CollectionId::id)
            .unwrap_or(current.collection_id),
