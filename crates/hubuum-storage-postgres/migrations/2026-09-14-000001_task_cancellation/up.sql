ALTER TABLE tasks
    ...
    ADD COLUMN cancel_requested_by INTEGER REFERENCES principals(id) ON DELETE SET NULL,
...
CREATE INDEX tasks_execution_deadline_active_idx ON tasks (execution_deadline_at, id)
    WHERE status IN ('queued', 'validating', 'running') AND execution_deadline_at IS NOT NULL;
