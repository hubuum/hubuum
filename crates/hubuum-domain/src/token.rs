...
pub struct TokenPolicyError(String);
...
impl TokenRetentionPeriod {
    pub fn from_days(days: i64) -> Result<Self, TokenPolicyError> {
        if days <= 0 {
        ...
        Duration::try_days(days).map(Self).ok_or_else(|| {
            TokenPolicyError::new("token_retention_days is outside the supported range".to_string())
        })
    }
...
impl TokenLifetime {
    ...
    fn from_hours_for(setting_name: &str, hours: i64) -> Result<Self, TokenPolicyError> {
        if hours <= 0 {
        ...
        Duration::try_hours(hours).map(Self).ok_or_else(|| {
            TokenPolicyError::new(format!("{setting_name} is outside the supported range"))
        })
    }
    ...
    pub fn cutoff_from(self, now: NaiveDateTime) -> Result<NaiveDateTime, TokenPolicyError> {
        now.checked_sub_signed(self.0).ok_or_else(|| {
            TokenPolicyError::new(
                "token lifetime cutoff is outside the supported range".to_string(),
            )
        })
    }

    fn expiry_from(self, issued_at: NaiveDateTime) -> Result<NaiveDateTime, TokenPolicyError> {
        issued_at.checked_add_signed(self.0).ok_or_else(|| {
            TokenPolicyError::new("token expiry is outside the supported range".to_string())
        })
    }
}
...
impl TokenIssuancePolicy {
    ...
    pub fn default_lifetime(self) -> TokenLifetime {
    ...
    ) -> Result<NaiveDateTime, TokenPolicyError> {
...
impl TokenRetentionSettings {
    ...
    pub fn cutoffs(self, now: NaiveDateTime) -> Result<TokenRetentionCutoffs, TokenPolicyError> {
...
impl TokenRetentionSettingsBuilder {
    ...
    pub fn build(self) -> Result<TokenRetentionSettings, TokenPolicyError> {
        let retention_days = self
            .retention_days
            .ok_or_else(|| TokenPolicyError::new("token_retention_days is required".to_string()))?;
        let token_lifetime_hours = self
            .token_lifetime_hours
            .ok_or_else(|| TokenPolicyError::new("token_lifetime_hours is required".to_string()))?;
        let batch_size = self.batch_size.ok_or_else(|| {
            TokenPolicyError::new("token_retention_purge_batch_size is required".to_string())
        })?;
