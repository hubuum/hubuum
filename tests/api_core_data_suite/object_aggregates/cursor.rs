...
async fn oversized_group_key_is_rejected_before_a_cursor_is_emitted(
    ...
    let large_value = format!("a{}", "x".repeat(MAX_OBJECT_AGGREGATE_CURSOR_LENGTH));
    let fixture = test_context
        .object_fixture(
            "oversized aggregate cursor",
            NewHubuumClass {
                collection_id: 0,
                name: test_context.scoped_name("oversized aggregate cursor class"),
                description: "Oversized aggregate cursor".to_string(),
                json_schema: None,
            ...
            vec![
                NewHubuumObject {
                    collection_id: 0,
                    hubuum_class_id: 0,
                    name: test_context.scoped_name("oversized aggregate cursor object"),
                ...
                NewHubuumObject {
                    collection_id: 0,
                    hubuum_class_id: 0,
                    name: test_context.scoped_name("small aggregate cursor object"),
...
#[rstest::rstest]
#[tokio::test]
async fn empty_class_returns_empty_aggregate_page(#[future(awt)] test_context: TestContext) {
    let fixture = test_context
        .object_fixture(
            "empty groups",
            NewHubuumClass {
...
#[rstest::rstest]
#[tokio::test]
async fn empty_aggregate_page_rejects_a_malformed_cursor(#[future(awt)] test_context: TestContext) {
    let fixture = test_context
        .object_fixture(
            "empty malformed aggregate cursor",
            NewHubuumClass {
                collection_id: 0,
                name: test_context.scoped_name("empty malformed aggregate cursor class"),
                description: "Empty".to_string(),
                json_schema: None,
                validate_schema: Some(false),
            },
...
#[rstest::rstest]
#[tokio::test]
async fn empty_aggregate_page_rejects_structurally_invalid_cursor(
    #[future(awt)] test_context: TestContext,
) {
    let fixture = fixture(&test_context, "invalid cursor ordering values").await;
    let cursor = encoded_aggregate_cursor(serde_json::json!([null]), 1);
    let response = get_request(
            &test_context.pool,
            &test_context.admin_token,
            &format!(
...
#[rstest::rstest]
#[tokio::test]
async fn empty_aggregate_page_rejects_a_cursor_for_different_dimensions(
    #[future(awt)] test_context: TestContext,
) {
    let fixture = fixture(&test_context, "mismatched empty aggregate cursor").await;
    let response = get_request(
        &test_context.pool,
        &test_context.admin_token,
        &format!(
