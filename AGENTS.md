...
- Python tooling and tests require Python 3.11 or newer and must use only the Python standard library. Ensure `python3` on `PATH` selects a supported interpreter. Do not add third-party Python packages, pip installation steps, or virtual environment requirements.
...
- Add at most one new migration directory/version per pull request. Consolidate
  all of that PR's database changes into its `up.sql` and `down.sql`; optional
  `metadata.toml` belongs to that same migration. Do not rewrite migrations
  that have already merged or shipped.
