...
for budget_input in src/config/schema_validation.rs src/tests/storage_contract/schema_evolution/budgets.rs; do
  budget_output="$(bash "$classifier" "$budget_input")"
  assert_flag "$budget_output" code true
done
