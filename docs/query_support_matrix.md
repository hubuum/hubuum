...
| `/api/v1/classes/{class_id}/object-aggregates` | non-computed object filters: `id`, `name`, `description`, `collections`, `collection_id`, `classes`, `class_id`, `json_data`, `created_at`, `updated_at`, `permissions` | `dimensions`, `object_count` | `dimensions.asc` | requires one to three repeated `group_by` dimensions; supports scalar object fields, nested `json_data` paths, enabled shared computed fields, and the requesting human's enabled personal computed fields; computed fields are dimensions only, not source filters; `X-Total-Count` reports aggregate cardinality |
...
`/api/v1/classes/{class_id}/related/graph` is not a paginated list endpoint. It accepts connected-class filters such as `depth` to define the included neighborhood and returns a graph object containing `classes` and `relations`.
