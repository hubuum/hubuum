...
| `/api/v1/classes/{class_id}/object-groups` | same object filters as `/api/v1/classes/{class_id}/` | `dimensions`, `object_count` | `dimensions.asc` | requires one to three repeated `group_by` dimensions; supports scalar object fields, nested `json_data` paths, enabled shared computed fields, and the requesting human's enabled personal computed fields; `X-Total-Count` reports group cardinality |
