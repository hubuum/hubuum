...
- `HubuumNamespace::"9201"` — the test namespace
...
// Normal group reads the test namespace.
permit(
    principal in Group::"9101",
    action == Action::"ReadCollection",
    resource
) when {
    resource is HubuumNamespace && resource == HubuumNamespace::"9201"
};
```
...
| `live_namespaces_user_can_reflects_external_policy` | namespace 9201 visible for Group 9101 with ReadCollection |
| `live_group_permission_on_returns_grant_grid_for_known_group` | synthesized Permission row has `has_read_namespace=true` for Group 9101 on namespace 9201 |
