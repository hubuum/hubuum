...
use crate::models::{
    ...
    HubuumObjectReadResponse, HubuumObjectRelation, HubuumObjectWithPath,
    NewHubuumClass, NewHubuumClassRelationFromClass, NewHubuumObjectRelation,
    NewHubuumObjectRequest, ObjectDataPatchDocument, ObjectSelector, Permissions,
...
async fn get_objects_in_class(
    ...
    let target = ClassSelector::by_id(class_id.into_inner())
        .resolve_class_target(&pool)
        .await?;
    list_objects_in_resolved_class(pool, requestor, target, req).await
}
...
async fn list_objects_in_resolved_class(
    pool: AppContext,
    requestor: Authenticated,
    target: ResolvedClassTarget,
    req: HttpRequest,
    ...
    let (objects, total_count) = if pool.permission_backend().supports_sql_visibility_pushdown() {
        ...
        if !scope_allows(requestor.scopes(), &[Permissions::ReadObject]) {
            ...
            return object_read_page(page, 0, effective_page_limit(&params)?, include_computed);
        }
        let candidates = user
    ...
    if include_computed {
        ...
        object_read_page(page, total_count, effective_page_limit(&params)?, false)
    }
}
