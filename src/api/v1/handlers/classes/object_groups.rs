...
use crate::models::{
    HubuumClassID, ObjectGroupAuthorization, ObjectGroupBackendRequest, ObjectGroupCursorBudget,
    ObjectGroupRow, ObjectGroupTarget, Permissions, UserID,
};
...
pub(crate) async fn get_object_aggregates(
    ...
    let cursor_budget =
        ObjectGroupCursorBudget::for_request_target(req.path(), req.query_string())?;
    ...
    let mut request = ObjectGroupBackendRequest::builder(target, query)
