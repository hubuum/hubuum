...
pub(super) const MAX_OBJECT_GROUP_ACCUMULATOR_BYTES: usize = 8 * 1024 * 1024;

#[derive(Debug, Clone, Copy)]
pub(super) enum ObjectGroupJsonBound {
    CandidateBatch,
    Accumulator,
}

impl ObjectGroupJsonBound {
    pub(super) const fn max_bytes(self) -> usize {
        match self {
            Self::CandidateBatch => MAX_OBJECT_GROUP_CANDIDATE_BATCH_BYTES,
            Self::Accumulator => MAX_OBJECT_GROUP_ACCUMULATOR_BYTES,
        }
    }
    ...
    pub(super) fn overflow_error(self) -> ApiError {
        match self {
            ...
            Self::Accumulator => ApiError::PayloadTooLarge(format!(
                "Object group cardinality and values exceed the {}-byte intermediate storage limit; narrow the source filters or grouping dimensions",
                self.max_bytes()
            )),
        }
    }
    ...
    const fn subject(self) -> &'static str {
        match self {
            Self::CandidateBatch => "object group candidate",
            Self::Accumulator => "grouped object row",
        }
    }
}
...
impl Write for BoundedSizeWriter {
    fn write(&mut self, buffer: &[u8]) -> io::Result<usize> {
        let Some(bytes) = self.bytes.checked_add(buffer.len()) else {
            self.exceeded = true;
            return Err(io::Error::other("object group serialized size overflowed"));
        };
        if bytes > self.max_bytes {
            self.exceeded = true;
            return Err(io::Error::other(
                "object group value exceeds its serialized size bound",
            ));
        }
        self.bytes = bytes;
        Ok(buffer.len())
