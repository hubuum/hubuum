...
#[actix_test]
async fn mock_authorizes_relation_via_from_to_collection_attrs() {
    let backend = MockTreetopBackend::new();
    // A rule that requires both from_collection_id == 5 AND to_collection_id == 6.
    backend.add_rule(MockAllowRule {
        group_id: 100,
...
#[actix_test]
async fn mock_task_policy_can_allow_a_non_owner() {
    let backend = MockTreetopBackend::new();
...
async fn mock_group_permission_on_synthesizes_row_from_rules() {
    ...
    let row = row.unwrap();
    assert!(
        row.has_read_collection,
        "ReadCollection grant should set has_read_collection"
    );
    assert!(
    ...
    assert!(
        !row.has_update_collection,
        "ungranted permissions stay false"
    );
    assert_eq!(row.id, 100, "synthetic row id is stable per group");
    assert_eq!(row.collection_id, 7);
...
async fn mock_groups_with_permissions_on_filters_and_paginates() {
    ...
    assert_eq!(second_page.items[0].group.id, 100);
}
