...
mod tests {
    ...
    async fn exported_cedar_grants_same_non_relation_decisions_as_local() {
        ...
        let user_alpha = crate::tests::create_test_user(&pool).await;
        let user_beta = crate::tests::create_test_user(&pool).await;
        let g_alpha = crate::tests::create_test_group(&pool).await;
        let g_beta = crate::tests::create_test_group(&pool).await;

        g_alpha
            .add_member(&pool, &user_alpha)
            .await
            .expect("add user_alpha to g_alpha");
        g_beta
            .add_member(&pool, &user_beta)
            .await
            .expect("add user_beta to g_beta");

        let ns_one =
            crate::tests::create_namespace_fixture(&pool, &format!("export_one_{unique}")).await;
        let ns_two =
            crate::tests::create_namespace_fixture(&pool, &format!("export_two_{unique}")).await;
        ...
        crate::permissions::export::export_cedar_to(&pool, &mut buf)
            .await
        ...
        let cases: Vec<(
            ...
            (
                ...
                alpha.clone(),
                req_namespace(ns_one.namespace.id, Permissions::ReadCollection),
                PermissionDecision::Allow,
            ),
            (
                "alpha cannot delete ns_one",
                alpha.clone(),
                req_namespace(ns_one.namespace.id, Permissions::DeleteCollection),
                PermissionDecision::Deny,
            ),
            (
                "alpha can read class on ns_one",
                alpha.clone(),
                req_class_on(ns_one.namespace.id, 12345, Permissions::ReadClass),
                PermissionDecision::Allow,
            ),
            (
                "alpha cannot read class on ns_two",
                alpha.clone(),
                req_class_on(ns_two.namespace.id, 12345, Permissions::ReadClass),
                PermissionDecision::Deny,
            ),
            (
                "alpha can read object on ns_two",
                alpha.clone(),
                req_object_on(ns_two.namespace.id, 67890, Permissions::ReadObject),
                PermissionDecision::Allow,
            ),
            (
                "alpha cannot read object on ns_one",
                alpha.clone(),
                req_object_on(ns_one.namespace.id, 67890, Permissions::ReadObject),
                PermissionDecision::Deny,
            ),
            (
                "beta can delegate ns_one",
                beta.clone(),
                req_namespace(ns_one.namespace.id, Permissions::DelegateCollection),
                PermissionDecision::Allow,
            ),
            (
    ...
    async fn exporter_relation_permits_emit_or_doubled_rules() {
        ...
        let unique = crate::utilities::auth::generate_random_password(8);
        let user_rel = crate::tests::create_test_user(&pool).await;
        let g_rel = crate::tests::create_test_group(&pool).await;
        g_rel
            .add_member(&pool, &user_rel)
            .await
            .expect("add user_rel to g_rel");

        let ns_rel =
            crate::tests::create_namespace_fixture(&pool, &format!("export_rel_{unique}")).await;
        ...
        let other_ns = ns_rel.namespace.id + 999_999;
        ...
        let same_ns_relation = PermissionRequest {
            resource: ResourceRef {
                ...
                id: 42,
                attrs: ResourceAttrs {
                    from_namespace_id: Some(ns_rel.namespace.id),
                    to_namespace_id: Some(ns_rel.namespace.id),
                    ..Default::default()
                },
            },
        ...
        let no_endpoint_relation = PermissionRequest {
            resource: ResourceRef {
                ...
                id: 44,
                attrs: ResourceAttrs {
                    from_namespace_id: Some(other_ns),
                    to_namespace_id: Some(other_ns),
                    ..Default::default()
                },
            },
    ...
    fn req_namespace(ns_id: i32, perm: Permissions) -> PermissionRequest {
        PermissionRequest {
            resource: ResourceRef::namespace(ns_id),
    ...
    fn parse_cedar_into_mock(cedar: &str, mock: &MockTreetopBackend) -> usize {
        ...
        for caps in permit_re.captures_iter(cedar) {
            ...
            if let Some(ns_id) = parse_collection_ns(when_clause) {
                ...
                // To express "from_namespace_id == N OR to_namespace_id == N",
                // emit TWO rules: one with from_namespace_id=Some(N), one with
                // to_namespace_id=Some(N). The mock evaluates rules
    ...
    fn parse_collection_ns(when: &str) -> Option<i32> {
        let re =
            Regex::new(r#"resource is HubuumNamespace && resource == HubuumNamespace::"(\d+)""#)
                .unwrap();
    ...
    fn parse_child_resource(when: &str) -> Option<(ResourceKind, i32)> {
        let re = Regex::new(
            r"resource is (HubuumClass|HubuumObject|HubuumTemplate) && resource\.namespace_id == (\d+)",
        )
        .unwrap();
