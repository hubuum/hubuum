...
use crate::models::{
    Group, GroupPermission, Collection, NewPermission, Permission, Permissions, PermissionsList,
    UpdatePermission, UserID,
};
use crate::schema::{collections as collections_schema, permissions as permissions_schema};
...
pub(crate) async fn user_can_all_query(
    ...
    let lookup_table = permissions_schema::table;
    let group_id_field = permissions_schema::group_id;
    let collection_id_field = permissions_schema::collection_id;
    let group_id_subquery = user.group_ids_subquery_from_backend();

    let mut base_query = lookup_table
        .into_boxed()
        .filter(collection_id_field.eq(collection_id))
...
pub(crate) async fn apply_permissions_query(
    ...
    with_transaction(pool, |conn| -> Result<Permission, ApiError> {
        let existing_entry = permissions
            .filter(collection_id.eq(collection_id_param))
...
pub(crate) async fn user_on_query<T: CollectionAccessors>(
    ...
    use crate::schema::permissions::dsl::{group_id, collection_id, permissions};

    let collection_target_id = collection_ref.collection_id(pool).await?;
...
pub(crate) async fn user_on_paginated_query<T: CollectionAccessors>(
    ...
    use crate::schema::permissions::dsl::{
        created_at as permission_created_at, group_id, id as permission_id, collection_id,
    ...
    apply_query_options!(query, query_options, GroupPermission);
...
pub(crate) async fn user_can_on_any_query(
    ...
    let user = UserID(user_id);
    if user.is_admin(pool).await? {
        return all_collections(pool).await;
    }

    let group_id_subquery = user.group_ids_subquery_from_backend();
    let mut filtered = permissions_schema::table
        .into_boxed()
        .filter(permissions_schema::group_id.eq_any(group_id_subquery));

    for permission in permissions_requested {
        filtered = permission.create_boxed_filter(filtered, true);
    }

    let accessible_collection_ids: Vec<i32> = with_connection(pool, |conn| {
        filtered
            .select(permissions_schema::collection_id)
            .load::<i32>(conn)
    })?;
    ...
    with_connection(pool, |conn| {
        collections_schema::table
            .filter(collections_schema::id.eq_any(accessible_collection_ids))
...
pub(crate) async fn group_can_on_query<T: CollectionAccessors>(
    ...
    let base_query = permissions
        ...
        .filter(collection_id.eq(collection_ref.collection_id(pool).await?));
...
pub(crate) async fn groups_can_on_paginated_with_total_count_query(
    ...
    let mut query = build_query()?;
    apply_query_options!(query, query_options, Group);
